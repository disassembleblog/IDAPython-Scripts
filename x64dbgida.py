...
class HandlerExportDB(idaapi.action_handler_t):
	def __init__(self):
...
action_exportdb = idaapi.action_desc_t(
	'my:exportdb',   # The action name. This acts like an ID and must be unique
	'ExportDB',  # The action text.
	HandlerExportDB(),   # The action handler.
	'',      # Optional: the action shortcut
	'',  # Optional: the action tooltip (available in menus/toolbar)
	)           # Optional: the action icon (shows when in menus/toolbars)

# 3) Register the action
idaapi.register_action(action_exportdb)


# 1) Create the handler class
class HandlerImportDB(idaapi.action_handler_t):
	def __init__(self):
		idaapi.action_handler_t.__init__(self)
...
# 2) Describe the action
action_importdb = idaapi.action_desc_t(
	'my:importdb',   # The action name. This acts like an ID and must be unique
	'ImportDB',  # The action text.
	HandlerImportDB(),   # The action handler.
	'',      # Optional: the action shortcut
	'',  # Optional: the action tooltip (available in menus/toolbar)
	)           # Optional: the action icon (shows when in menus/toolbars)

# 3) Register the action
idaapi.register_action(action_importdb)
